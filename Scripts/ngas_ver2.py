...
AT0 = 10500; ATF = 11000; INCREMENT_AT = 50
TARGETS = np.arange(AT0, ATF+INCREMENT_AT, INCREMENT_AT)


GAS = GASES[MOL]
print(f'mol: {MOL}, ngas: {NGAS}')
data = []
...
for i,TARGET in enumerate(tqdm(TARGETS)):
    ...
    #mean_nw = np.mean(nw)
    #std_nw = np.std(nw)
    data.append([MOL, natoms, nwater, NGAS, TARGET])

if data:
    df = pd.DataFrame(data, columns=["mol","natoms","nwater","ngas","coord"])
